...
environment to a Redis bibliograhic datastore based on a BIBFRAME Datstore
set-up. The Aristotle Library Apps project also included interfaces to a 
Fedora Commons Server for digital object storage, and a Solr Interface.
...
1. Create a virtualenv for running Aristotle Library Apps environment with
   the following dependancies installed in the virtualenv:
...
2. Clone Aristotle Library Apps project from GitHub
...
3. Create or modify a local_settings.py file in the aristotle app directory.
   Make sure that it includes the following settings variables:
      ...
      REDIS_MASTER_HOST = '0.0.0.0' # Or other IP address of running 
      Redis instance

      SOLR_URL
      

4. Follow directions for creating a database for Django admin and limited used
